...
const CollectionsList = ({
  ...
  const handleDelete = () => {
    deleteCollections([selectedRowId]).then(res => {
  ...
  const columns = [
    ...
    },
    {
      ...
      minWidth: 120,
      field: 'parent_id',
      headerName: t('parent_Collection'),
      renderCell: ({ row }) => (
        <Typography variant='body2' sx={{ color: 'text.primary' }}>
          {row.parent?.name}
        </Typography>
    ...
    },
    {
      ...
      renderCell: ({ row }) => <CollectionsRowOptions id={row.id} handleClickDeleteButton={handleClickDeleteButton} />
    }
  ]
  ...
  return (
    <div>
      ...
      <Card>
        ...
        <CustomDataGrid
          toolbar={CollectionsListTableHeader}
          toolbarProps={{
            value: search,
