...
const MaterialItemDetails = ({ type }) => {
  const { t } = useTranslation()
  const router = useRouter()
  const [openDeleteSnackbar, setOpenDeleteSnackbar] = useState(false)

  const handleDelete = () => {
    deleteMaterialItem([type.id]).then(res => {
      toast.success(t('success'))
      setOpenDeleteSnackbar(false)
      router.replace('/material-items')
    })
  }
      ...
      <Grid item xs={12}>
        <Card>
          ...
          <CardContent>
            ...
            <Box sx={{ display: 'flex', mb: 3 }}>
              <Typography sx={{ mr: 2, fontWeight: 500, color: 'text.secondary' }}>{t('material_folder')}:</Typography>
              <Typography sx={{ color: 'text.secondary' }}>{type.material_folder?.name}</Typography>
            </Box>
            <Box sx={{ display: 'flex', mb: 3 }}>
          ...
          <CardActions sx={{ display: 'flex', justifyContent: 'center' }}>
            <Button variant='tonal' sx={{ mr: 2 }} onClick={() => router.push(`/material-items/edit/${type.id}`)}>
              {t('edit')}
            </Button>
...
export default MaterialItemDetails
