...
const CollectionsListTableHeader = ({ selectedRows, onChange, value, clearSearch, fetchData, canExport }) => {
  ...
  const handleDelete = () => {
    deleteCollections(selectedRows).then(res => {
      setOpenDeleteSnackbar(false)
      fetchData()
    })
  }
  ...
  return (
    ...
    >
      ...
      <Box sx={{ rowGap: 2, display: 'flex', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        />
        <Button onClick={() => router.push('/collections/create')} variant='contained' sx={{ '& svg': { mr: 2 } }}>
...
export default CollectionsListTableHeader
