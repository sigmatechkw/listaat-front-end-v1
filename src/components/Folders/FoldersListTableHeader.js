...
const FoldersTableHeader = ({ selectedRows, onChange, value, clearSearch, fetchData, canExport }) => {
  ...
  const handleDelete = () => {
    deleteFolders(selectedRows).then(res => {
      setOpenDeleteSnackbar(false)
      fetchData()
    })
  }
  ...
  return (
    ...
    >
      ...
      <Box sx={{ rowGap: 2, display: 'flex', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        />
        <Button onClick={() => router.push('/folders/create')} variant='contained' sx={{ '& svg': { mr: 2 } }}>
...
export default FoldersTableHeader
