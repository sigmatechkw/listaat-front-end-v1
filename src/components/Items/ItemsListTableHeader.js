...
import { deleteItems } from './itemsServices'
import { useRouter } from 'next/router'

const ItemsListTableHeader = ({ selectedRows, onChange, value, clearSearch, fetchData, canExport }) => {
  ...
  const handleDelete = () => {
    deleteItems(selectedRows).then(res => {
      setOpenDeleteSnackbar(false)
      fetchData()
    })
  }
  ...
  return (
    ...
    >
      ...
      <Box sx={{ rowGap: 2, display: 'flex', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        />
        <Button onClick={() => router.push('/items/create')} variant='contained' sx={{ '& svg': { mr: 2 } }}>
...
export default ItemsListTableHeader
