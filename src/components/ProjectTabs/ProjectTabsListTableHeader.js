...
const ProjectTabsListTableHeader = ({ selectedRows, onChange, value, clearSearch, fetchData, canExport }) => {
  ...
  const handleDelete = () => {
    deleteProjectTabs(selectedRows).then(res => {
      setOpenDeleteSnackbar(false)
      fetchData()
    })
  }
  ...
  return (
    ...
    >
      ...
      <Box sx={{ rowGap: 2, display: 'flex', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        />
        <Button onClick={() => router.push('/project-tabs/create')} variant='contained' sx={{ '& svg': { mr: 2 } }}>
...
export default ProjectTabsListTableHeader
