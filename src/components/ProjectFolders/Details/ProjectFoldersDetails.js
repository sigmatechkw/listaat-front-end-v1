...
import { deleteProjectFolders } from '../ProjectFoldersServices'
...
const ProjectFoldersDetails = ({ type }) => {
  const { t } = useTranslation()
  const router = useRouter()
  const [openDeleteSnackbar, setOpenDeleteSnackbar] = useState(false)

  const handleDelete = () => {
    deleteProjectFolders([type.id]).then(res => {
      toast.success(t('success'))
      setOpenDeleteSnackbar(false)
      router.replace('/project-folders')
    })
  }
        ...
        <Card>
          ...
          <CardContent>
            <Box sx={{ display: 'flex', mb: 3 }}>
              <Typography sx={{ mr: 2, fontWeight: 500, color: 'text.secondary' }}>{t('project_id')}:</Typography>
              <Typography sx={{ color: 'text.secondary' }}>{type.project?.name}</Typography>
            </Box>
            <Box sx={{ display: 'flex', mb: 3 }}>
              <Typography sx={{ mr: 2, fontWeight: 500, color: 'text.secondary' }}>{t('folder')}:</Typography>
              <Typography sx={{ color: 'text.secondary' }}>{type.folder?.name}</Typography>
            </Box>
            <Box sx={{ display: 'flex', mb: 3 }}>
...
export default ProjectFoldersDetails
