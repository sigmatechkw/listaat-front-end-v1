...
const FoldersDetails = ({ type }) => {
  const { t } = useTranslation()
  const router = useRouter()
  const [openDeleteSnackbar, setOpenDeleteSnackbar] = useState(false)

  const handleDelete = () => {
    deleteFolders([type.id]).then(res => {
      ...
      <Grid item xs={12}>
        <Card>
          ...
          <CardContent>
            ...
            <Box sx={{ display: 'flex', mb: 3 }}>
              <Typography sx={{ mr: 2, fontWeight: 500, color: 'text.secondary' }}>{t('parent_folder')}:</Typography>
          ...
          <CardActions sx={{ display: 'flex', justifyContent: 'center' }}>
            <Button variant='tonal' sx={{ mr: 2 }} onClick={() => router.push(`/folders/edit/${type.id}`)}>
              {t('edit')}
            </Button>
