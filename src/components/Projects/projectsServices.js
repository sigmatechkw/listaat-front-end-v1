...
export const fetchProjects = async (page = 1, search, sortKey = 'id', sortType = 'desc', perPage = 10, isActive = '', setRows, setLoading) => {
  let params = {
    paginate: 1,
    page: page + 1,
    perPage,
...
export const deleteProjects = async (ids) => {
  ...
  try {
    await axios.post(`${process.env.NEXT_PUBLIC_API_KEY}projects/delete`, data, {
      headers: {
        'Authorization': getCookie('token'),
        'Accepted-Language': getCookie('lang') ?? state.lang ?? 'en'
      }
