...
const ThemeFolderDetails = ({ type }) => {
  const { t } = useTranslation()
  const router = useRouter()
  const [openDeleteSnackbar, setOpenDeleteSnackbar] = useState(false)

  const handleDelete = () => {
    deleteThemeFolders([type.id]).then(res => {
      toast.success(t('success'))
      setOpenDeleteSnackbar(false)
      router.replace('/theme-folders')
    })
  }
      ...
      <Grid item xs={12}>
        <Card>
          ...
          <CardActions sx={{ display: 'flex', justifyContent: 'center' }}>
            <Button variant='tonal' sx={{ mr: 2 }} onClick={() => router.push(`/theme-folders/edit/${type.id}`)}>
              {t('edit')}
            </Button>
...
export default ThemeFolderDetails
